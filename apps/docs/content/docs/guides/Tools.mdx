...
- **`makeAssistantTool`**: Complete tools with execution logic and optional UI
- **`useAssistantTool`**: Hook-based dynamic tool registration
- **`makeAssistantToolUI`**: UI-only components for existing tools
- **Inline registration**: Direct tool registration with the runtime

### 1. Using `makeAssistantTool`

Create a complete tool with execution logic and optional UI. Returns a React component that registers the tool UI when rendered:

<Callout type="info">
**Important:** The component returned by `makeAssistantTool` must be rendered inside `<AssistantRuntimeProvider>` to register its UI. The tool's execution logic runs where you define it, but the UI registration happens when the component mounts.
</Callout>

```tsx
import { makeAssistantTool, AssistantTool } from "@assistant-ui/react";
import { z } from "zod";
...
const WeatherTool = makeAssistantTool<{
  ...
  render: ({ args, result, status }) => {
    // Custom UI for tool display
    if (status.type === "running") return <div>Fetching weather...</div>;
    ...
    if (status.type === "running") {
      return (
        <div className="approval-pending">
          <p>Awaiting approval for refund:</p>
          <p>Order: {args.orderId}</p>
  ...
  render: ({ status, result, args }) => {
    if (status.type === "incomplete" && status.reason === "error") {
      return (
        <ErrorCard 
          error={status.error}
          retry={() => window.assistant.retryTool(args)}
        />
      );
    }
    // ... handle other states
...
- **AI SDK**: Tools defined in API routes with `streamText({ tools: {...} })` - execution happens server-side
...
- **LocalRuntime**: Tools passed via context or registered with `makeAssistantTool`
- **External Store**: Tools registered on the runtime with `runtime.registerTool()`

All runtimes support the same frontend tool UI components - just ensure the UI components are rendered inside `<AssistantRuntimeProvider>`. See the [runtime documentation](/docs/runtimes/pick-a-runtime) for specific examples.
