...
### 1. Complete Tool with UI (`makeAssistantTool`)

If you're creating a new tool from scratch, you can use `makeAssistantTool` to define both the tool's execution logic and its UI in one place. This is ideal when you control both the frontend and backend:
...
- [Mastra Integration](/docs/runtimes/mastra/tool-ui-example) - Build generative UI with Mastra agents and tools
